...
echo "---------------------------------------------"
# Change root password to be able to register
echo "root:${12}" | sudo chpasswd
# Register Server with redhat
echo "sudo /usr/sbin/subscription-manager register --username ${10} --password ${11} --auto-attach"
sudo /usr/sbin/subscription-manager register --username ${10} --password ${11} --auto-attach
